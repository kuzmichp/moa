...
public class ModelImpl implements Model {
	...
	private final EvolutionMonitor evolutionMonitor;

	private Genotype<DoubleGene> genotype;

	private ModelImpl(int populationSize, int maxEpochs, int eliteCount, int steadiness, int limit, int k) {
	...
	public static class Builder {
		...
		public Model build() {
			return new ModelImpl(populationSize, maxEpochs, eliteCount, steadiness, limit, k);
		}
	...
	private EvolutionResult<DoubleGene, Double> createModel(int individualSize, List<Instance> training,
		...
		Engine<DoubleGene, Double> engine = Engine.builder(g -> eval(g, training, test), factory)
				...
				.survivorsSelector(new EliteSelector<>(eliteCount))
				.alterers(new Mutator<>(0.015))
				.build();
		...
		EvolutionResult<DoubleGene, Double> evolutionResult = engine.stream()
				...
				.limit(maxEpochs)
				.peek(statistics)
				.peek(evolutionMonitor::monitorBestGenotype)
				.peek(evolutionMonitor::monitorPopulation)
